...
import { IntegrationCard, Button, Input, Switch, Spinner } from '@/components/ui';
...
export function PaymentIntegrationsTab({ isLoading: externalLoading }: PaymentIntegrationsTabProps) {
  ...
  // Get status for IntegrationCard
  const getStatus = (integration: PaymentIntegration | undefined): 'connected' | 'disconnected' | 'error' => {
    if (!integration) return 'disconnected';
    if (integration.verification_status === 'failed') return 'error';
    if (integration.is_enabled) return 'connected';
    return 'disconnected';
  };
    ...
    <div className="space-y-4">
      ...
      <div className="space-y-4">
        ...
        <IntegrationCard
          ...
          logo={<PaystackLogo />}
          status={getStatus(getIntegration('paystack'))}
          isPrimary={getIntegration('paystack')?.is_primary}
          isExpanded={expandedProvider === 'paystack'}
          onToggle={() => setExpandedProvider(expandedProvider === 'paystack' ? null : 'paystack')}
        >
          <div className="space-y-4">
            {/* Enable toggle */}
            <div className="flex items-center justify-between">
              <span className="text-sm font-medium text-gray-700 dark:text-gray-300">Enable Paystack</span>
              <Switch
                checked={getIntegration('paystack')?.is_enabled || false}
                onCheckedChange={(checked) => handleToggleEnabled('paystack', checked)}
              />
            </div>
            ...
            <div>
              ...
              <div className="flex gap-2">
                <Button
                  size="sm"
                  variant={paystackForm.environment === 'test' ? 'primary' : 'outline'}
                ...
                <Button
                  size="sm"
                  variant={paystackForm.environment === 'live' ? 'primary' : 'outline'}
            ...
            {webhookUrls && (
              <div>
                ...
                <div className="flex items-center gap-2">
                  <code className="flex-1 px-3 py-2 bg-gray-100 dark:bg-gray-800 rounded text-sm font-mono text-gray-600 dark:text-gray-400 truncate">
                    {webhookUrls.paystack}
                  </code>
                  <Button
                    size="sm"
                    variant="outline"
                    onClick={() => copyToClipboard(webhookUrls.paystack)}
                  >
                    <CopyIcon />
                  </Button>
                </div>
                <p className="mt-1 text-xs text-gray-500">Add this URL to your Paystack dashboard webhook settings</p>
              </div>
            )}
            ...
            <div className="flex justify-end gap-2 pt-4 border-t border-gray-200 dark:border-gray-700">
              <Button
                ...
                disabled={testing === 'paystack' || !paystackForm.secret_key}
              >
                {testing === 'paystack' ? <Spinner size="sm" /> : 'Test Connection'}
              </Button>
              <Button
                variant="primary"
                onClick={() => handleSave('paystack')}
                disabled={saving === 'paystack'}
              >
                {saving === 'paystack' ? <Spinner size="sm" /> : 'Save Configuration'}
              </Button>
            </div>
          </div>
        </IntegrationCard>
        ...
        <IntegrationCard
          id="paypal"
          name="PayPal"
          description="Accept payments globally via PayPal accounts and credit/debit cards."
          logo={<PayPalLogo />}
          status={getStatus(getIntegration('paypal'))}
          isExpanded={expandedProvider === 'paypal'}
          onToggle={() => setExpandedProvider(expandedProvider === 'paypal' ? null : 'paypal')}
        >
          <div className="space-y-4">
            {/* Enable toggle */}
            <div className="flex items-center justify-between">
              <span className="text-sm font-medium text-gray-700 dark:text-gray-300">Enable PayPal</span>
              <Switch
                checked={getIntegration('paypal')?.is_enabled || false}
                onCheckedChange={(checked) => handleToggleEnabled('paypal', checked)}
              />
            </div>
            ...
            <div>
              ...
              <div className="flex gap-2">
                <Button
                  size="sm"
                  variant={paypalForm.environment === 'test' ? 'primary' : 'outline'}
                ...
                <Button
                  size="sm"
                  variant={paypalForm.environment === 'live' ? 'primary' : 'outline'}
              ...
              <div>
                ...
                <div className="flex items-center gap-2">
                  ...
                  <Button
                    size="sm"
                    variant="outline"
                    onClick={() => copyToClipboard(webhookUrls.paypal)}
                  >
                    <CopyIcon />
                  </Button>
                </div>
                <p className="mt-1 text-xs text-gray-500">Add this URL to your PayPal developer dashboard</p>
              </div>
            )}
            ...
            <div className="flex justify-end gap-2 pt-4 border-t border-gray-200 dark:border-gray-700">
              <Button
                ...
                disabled={testing === 'paypal' || !paypalForm.client_id || !paypalForm.client_secret}
              >
                {testing === 'paypal' ? <Spinner size="sm" /> : 'Test Connection'}
              </Button>
              <Button
                variant="primary"
              ...
              >
                {saving === 'paypal' ? <Spinner size="sm" /> : 'Save Configuration'}
              </Button>
            </div>
          </div>
        </IntegrationCard>
        ...
        <IntegrationCard
          ...
          description="Accept manual bank transfers with reference tracking."
          logo={<BankIcon />}
          status={getStatus(getIntegration('eft'))}
          isExpanded={expandedProvider === 'eft'}
          onToggle={() => setExpandedProvider(expandedProvider === 'eft' ? null : 'eft')}
        >
          <div className="space-y-4">
            {/* Enable toggle */}
            <div className="flex items-center justify-between">
              <span className="text-sm font-medium text-gray-700 dark:text-gray-300">Enable EFT Payments</span>
              <Switch
                checked={getIntegration('eft')?.is_enabled || false}
                onCheckedChange={(checked) => handleToggleEnabled('eft', checked)}
              />
            </div>
            ...
            <div className="flex justify-end gap-2 pt-4 border-t border-gray-200 dark:border-gray-700">
              <Button
                variant="outline"
                onClick={() => handleTestConnection('eft')}
                disabled={testing === 'eft'}
              >
                {testing === 'eft' ? <Spinner size="sm" /> : 'Validate Configuration'}
              </Button>
              <Button
                variant="primary"
                onClick={() => handleSave('eft')}
                disabled={saving === 'eft'}
              >
                {saving === 'eft' ? <Spinner size="sm" /> : 'Save Configuration'}
              </Button>
            </div>
          </div>
        </IntegrationCard>
