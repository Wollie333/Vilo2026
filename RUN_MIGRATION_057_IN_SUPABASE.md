# Apply Migration 057 - Property-Level Payment Rules
...
Expected output: Should show the check constraint definition

---

## What This Fixes

This migration resolves the 500 error when trying to fetch payment rules in the Room Wizard by:

1. Adding support for **property-level payment rules** (not tied to a specific room)
2. Allowing `room_id` to be null for property-level rules
3. Ensuring either `room_id` OR `property_id` is set (but not both)
4. Updating RLS policies to work with both rule types

After applying this migration, the Payment Rules step in the Room Wizard will be able to:
- Fetch existing payment rules for the property
- Display them as selectable cards
- Allow creating new property-level rules
- Assign rules to rooms during creation/editing
