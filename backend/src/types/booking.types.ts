...
export type BookingStatus =
  | 'pending'
  | 'confirmed'
  | 'pending_modification' // NEW: Admin made changes, awaiting guest approval
  | 'checked_in'
  | 'checked_out'
  | 'completed'
  | 'cancelled'
...
export type PaymentStatus =
  | 'pending'
  | 'failed_checkout' // NEW: Checkout abandoned/failed (90-day retention)
  | 'verification_pending' // NEW: EFT payment needs verification
  | 'partial'
  | 'paid'
  | 'refunded'
  | 'partially_refunded' // NEW: Partial refund issued
  | 'failed';
...
export interface CreateBookingRoomRequest {
  ...
  children_ages?: number[];
  unit_number?: number;
  // Nightly rates will be calculated based on room and dates
}
...
export interface Booking {
  ...
  balance_due: number; // NEW: Outstanding balance to be paid
  ...
  total_refunded: number; // Amount refunded for this booking

  // Failed Checkout Tracking (NEW)
