...
-- Delete subscriptions (just delete all non-super-admin)
...
-- Delete user-related data (using safe approach - delete ALL since we'll keep super admin users)
-- These tables might have different column names, so just delete all
...
-- Delete user_roles (except super admin)
